...
// Setup tests must NOT inherit storageState from the dependent chromium project
...
setup('save student auth state', async ({ page }) => {
  await loginAndSave(page, STUDENT.email, STUDENT.password, STUDENT_STATE, '/dashboard')
})

setup('save teacher auth state', async ({ page }) => {
  await loginAndSave(page, TEACHER.email, TEACHER.password, TEACHER_STATE, '/teacher')
})

setup('save admin auth state', async ({ page }) => {
  await loginAndSave(page, ADMIN.email, ADMIN.password, ADMIN_STATE, '/admin')
})
